...
// ============================================================
// ADead-BIB — Unified C Compiler CLI
// ============================================================
...
fn real_main(args: &[String]) -> Result<ExitCode, Box<dyn std::error::Error>> {
    ...
    match args[1].as_str() {
        ...
        // ── Version ─────────────────────────────────────
        "version" | "--version" | "-v" => {
            println!("{}", term::banner("C Compiler", VERSION));
            println!("  Languages: C (complete)");
        ...
        _ => {
            ...
            let first = &args[1];
            if first.ends_with(".c") || first.ends_with(".h") {
...
fn parse_request(args: &[String]) -> Result<CompileRequest, Box<dyn std::error::Error>> {
...
fn parse_request_step(args: &[String]) -> Result<CompileRequest, Box<dyn std::error::Error>> {
    let mut req = parse_request(args)?;
    req.step_mode = true;
    Ok(req)
}
...
fn print_usage(bin: &str) {
    ...
    println!("  {}  {} <file.c>                   {}", term::phase_header("SHORT:"), bin, term::dim("(auto-detect)"));
    println!();
    println!("  {}", term::phase_header("COMMANDS:"));
    println!("    {}   <file.c>     Compile C source (C99/C11)", term::ok("cc  "));
    println!("    {}   <file>       Compile + run", term::ok("run "));
