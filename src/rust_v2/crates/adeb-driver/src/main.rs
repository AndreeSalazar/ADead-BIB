...
// ║                                                                              ║
// ║              ═══════════════════════════════════════════                     ║
// ║                   C/C++ Native Compiler CLI v9.0                             ║
// ║              Sin GCC · Sin LLVM · Sin Clang · 100% Autosuficiente            ║
// ║              ═══════════════════════════════════════════                     ║
// ║                                                                              ║
...
use adeb_backend_gpu::vulkan::VulkanBackend;
use adeb_backend_gpu::vulkan_runtime;
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match command.as_str() {
        ...
        "--test-lexer" => {
            ...
            let mut lexer = adeb_frontend_c::lexer::CLexer::new(&source);
            loop {
                let t = lexer.next_token();
                println!("line: {} token: {:?}", lexer.line, t);
                if t == adeb_frontend_c::lexer::CToken::Eof { break; }
            }
        }
        ...
        "cuda" => {
            ...
            use adeb_backend_gpu::cudead::CudeadCompiler;
        ...
        "unified" | "uni" => {
            use adeb_backend_gpu::unified_pipeline::{MathOperation, PipelineMode, UnifiedPipeline};
            use adeb_backend_x64::backend::{Backend, BackendMode};
...
fn compile_c_file(input_file: &str, args: &[String]) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut ub_detector = adeb_middle::UBDetector::new().with_file(input_file.to_string());
    if warn_ub {
    ...
    let mut compiler = if is_flat16 {
        adeb_backend_x64::isa::isa_compiler::IsaCompiler::new_real16()
    } else if is_flat64 || is_flat {
        // 64-bit long mode for flat binaries (bare metal kernel)
        adeb_backend_x64::isa::isa_compiler::IsaCompiler::new_long64(Target::Raw)
    } else {
        adeb_backend_x64::isa::isa_compiler::IsaCompiler::new(target)
    };
    ...
    if is_fastos_target(args) {
        use adeb_backend_x64::po::PoOutput;
        ...
        use adeb_backend_x64::flat_binary::FlatBinaryGenerator;
        ...
        match target {
            ...
            Target::Linux => {
                adeb_backend_x64::elf::generate_elf(&opcodes, &data, &output_file)?;
            }
            Target::Raw => {
...
fn print_cpp_banner() {
    ...
    println!("{}{}║                                                                              ║{}", BLUE, BOLD, RESET);
    println!("{}{}║              ═══════════════════════════════════════════                     ║{}", BLUE, BOLD, RESET);
    println!("{}{}║                   C++ Native Compiler CLI v9.0                               ║{}", BLUE, BOLD, RESET);
    println!("{}{}║              Sin GCC · Sin LLVM · Sin Clang · 100% Autosuficiente            ║{}", BLUE, BOLD, RESET);
    println!("{}{}║              ═══════════════════════════════════════════                     ║{}", BLUE, BOLD, RESET);
    println!("{}{}║                                                                              ║{}", BLUE, BOLD, RESET);
    println!("{}{}║              \"Respetar Bits\" — FORTRAN 1957 + Ada 1983 + ADead-BIB 2025    ║{}", BLUE, BOLD, RESET);
    println!("{}{}║                          Lima, Perú                                          ║{}", BLUE, BOLD, RESET);
    println!("{}{}║                                                                              ║{}", BLUE, BOLD, RESET);
    println!("{}{}╚══════════════════════════════════════════════════════════════════════════════╝{}", BLUE, BOLD, RESET);
    println!();
}
...
fn compile_cpp_file(input_file: &str, args: &[String]) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut compiler = adeb_backend_x64::isa::isa_compiler::IsaCompiler::new(target);
    let (opcodes, data, iat_offsets, string_offsets) = compiler.compile(&program);
...
fn print_js_banner() {
    ...
    println!("{}{}║                                                                              ║{}", JS_GOLD, BOLD, RESET);
    println!("{}{}║              ═══════════════════════════════════════════                     ║{}", JS_GOLD, BOLD, RESET);
    println!("{}{}║              JsDead-BIB — JavaScript → ASM Compiler v9.0                     ║{}", JS_GOLD, BOLD, RESET);
    println!("{}{}║         Sin Node.js · Sin V8 · Sin Runtime · 100% Autosuficiente             ║{}", JS_GOLD, BOLD, RESET);
    println!("{}{}║              ═══════════════════════════════════════════                     ║{}", JS_GOLD, BOLD, RESET);
    println!("{}{}║                                                                              ║{}", JS_GOLD, BOLD, RESET);
    println!("{}{}║              \"Respetar Bits\" — Brendan Eich 1995 → ASM 2026                ║{}", JS_GOLD, BOLD, RESET);
    println!("{}{}║                          Lima, Perú                                          ║{}", JS_GOLD, BOLD, RESET);
    println!("{}{}║                                                                              ║{}", JS_GOLD, BOLD, RESET);
    println!("{}{}╚══════════════════════════════════════════════════════════════════════════════╝{}", JS_GOLD, BOLD, RESET);
    println!();
}
...
fn install_global_headers() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Write header_main.h
    let header_main_content = adeb_frontend_c::stdlib::get_header("header_main.h")
    ...
    for name in &fastos_headers {
        if let Some(content) = adeb_frontend_c::stdlib::get_header(name) {
...
fn create_project(name: &str, is_cpp: bool) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let header_content = adeb_frontend_c::stdlib::get_header("header_main.h")
...
fn step_compile_c(input_file: &str) -> Result<(), Box<dyn std::error::Error>> {
    use adeb_core::cli::term;
    use adeb_frontend_c::lexer::{CLexer, CToken};
    use adeb_frontend_c::preprocessor::CPreprocessor;
    use adeb_frontend_c::parser::CParser;
    use adeb_frontend_c::c_to_ir::CToIR;
    use adeb_frontend_c::ast::CTopLevel;
    ...
    let target = adeb_backend_x64::isa::isa_compiler::Target::Windows;
    let mut compiler = adeb_backend_x64::isa::isa_compiler::IsaCompiler::new(target);
    let (opcodes, data, iat_offsets, string_offsets) = compiler.compile(&program);
    ...
    {
        let dlls = adeb_backend_x64::backend::cpu::iat_registry::dll_names();
        if !dlls.is_empty() {
            println!("  {} imports ({} DLLs):", term::dim("  IAT"), dlls.len());
            for dll in &dlls {
                let entries = adeb_backend_x64::backend::cpu::iat_registry::entries_for_dll(dll);
    ...
    let iat_section_size = adeb_backend_x64::backend::cpu::iat_registry::IAT_ENTRIES.len() * 8;
...
fn step_compile_cpp(input_file: &str) -> Result<(), Box<dyn std::error::Error>> {
    use adeb_core::cli::term;
    use adeb_frontend_cpp::lexer::{CppLexer, CppToken};
    ...
    let program = match adeb_frontend_cpp::compile_cpp_to_program(&source) {
        Ok(p) => {
    ...
    println!("{}", term::phase_bar(6, "IAT RESOLVER â€” Import Address Table", "C++"));
    {
        ...
        let idata_result = adeb_backend_x64::backend::cpu::iat_registry::build_idata(assumed_idata_rva, &[]);
        let iat_base = 0x2000 + idata_result.iat_offset as u32;

        let mut resolved_count = 0;
        for dll in adeb_backend_x64::backend::cpu::iat_registry::dll_names() {
            let entries = adeb_backend_x64::backend::cpu::iat_registry::entries_for_dll(&dll);
...
fn step_compile_js(input_file: &str) -> Result<(), Box<dyn std::error::Error>> {
    use adeb_core::cli::term;
    use adeb_frontend_js::lexer::JsLexer;
    use adeb_frontend_js::parser::JsParser;
    use adeb_frontend_js::js_to_ir::JsToIR;
    ...
    for stmt in &js_program.stmts {
        match stmt {
            adeb_frontend_js::ast::JsStmt::FuncDecl { .. } => func_count += 1,
            adeb_frontend_js::ast::JsStmt::ClassDecl { .. } => class_count += 1,
            adeb_frontend_js::ast::JsStmt::VarDecl { .. } => var_count += 1,
            _ => other_count += 1,
        }
    }
