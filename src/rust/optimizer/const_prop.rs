...
impl ConstPropagator {
    ...
    fn propagate_stmts(&self, stmts: &[Stmt], constants: &mut HashMap<String, i64>) -> Vec<Stmt> {
        stmts.iter().map(|s| self.propagate_stmt(s, constants)).collect()
    }

    fn propagate_stmt(&self, stmt: &Stmt, constants: &mut HashMap<String, i64>) -> Stmt {
        match stmt {
            // int x = 5; → track x=5
            Stmt::VarDecl { var_type: _, name, value: Some(Expr::Number(n)) } => {
            ...
            Stmt::If { condition, then_body, else_body } => {
                Stmt::If {
                    ...
                    else_body: else_body.as_ref().map(|eb| self.propagate_stmts(eb, &mut constants.clone())),
                }
            }
    ...
    fn propagate_expr(&self, expr: &Expr, constants: &HashMap<String, i64>) -> Expr {
        match expr {
            ...
            Expr::Call { name, args } => {
                Expr::Call {
                    name: name.clone(),
                    args: args.iter().map(|a| self.propagate_expr(a, constants)).collect(),
                }
            }
            _ => expr.clone(),
        }
