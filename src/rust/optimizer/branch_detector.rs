...
pub enum BranchPattern {
    ...
    // if (cond) a else b
    Select { cond: Expr, true_val: Expr, false_val: Expr, target: String },
    
    // if (x < min) x = min; if (x > max) x = max;
    Clamp { var: String, min: Expr, max: Expr },
}
...
impl BranchDetector {
    ...
    fn detect_relu(&self, stmt: &Stmt) -> Option<BranchPattern> {
        if let Stmt::If { condition, then_body, else_body } = stmt {
            // Check condition: x > 0
            if let Expr::Comparison { op: CmpOp::Gt, left, right } = condition {
                if let (Expr::Variable(var_name), Expr::Number(0)) = (left.as_ref(), right.as_ref()) {
                    // Check then: result = x
                    if then_body.len() == 1 {
                        if let Stmt::Assign { name: target_name, value: Expr::Variable(val_name) } = &then_body[0] {
                            if var_name == val_name {
                                // Check else: result = 0
                                if let Some(else_stmts) = else_body {
                                    if else_stmts.len() == 1 {
                                        if let Stmt::Assign { name: else_target, value: Expr::Number(0) } = &else_stmts[0] {
    ...
    fn detect_select(&self, stmt: &Stmt) -> Option<BranchPattern> {
        if let Stmt::If { condition, then_body, else_body } = stmt {
             if then_body.len() == 1 {
                 if let Stmt::Assign { name: target_name, value: true_expr } = &then_body[0] {
                     if let Some(else_stmts) = else_body {
                         if else_stmts.len() == 1 {
                             if let Stmt::Assign { name: else_target, value: false_expr } = &else_stmts[0] {
                                 if target_name == else_target {
