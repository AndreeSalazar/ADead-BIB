...
use crate::middle::ir::{
    Function, Module, Type as IRType, GlobalVariable, Instruction,
    Value, Constant, ValueId, BinaryOp, CompareOp, CastOp,
};
use crate::middle::ir::basicblock::BasicBlockId;
use crate::frontend::ast::{
    Function as AstFunction, Stmt, Expr, BinOp, CmpOp, UnaryOp,
};
    ...
    fn lower_stmt(&mut self, stmt: &Stmt) {
        match stmt {
            ...
            Stmt::For { var, start, end, body } => {
                ...
                self.emit(Instruction::load(IRType::I64, Value::Instruction(alloca_id), load_id));
                let end_val = self.lower_expr(end, &IRType::I64);
                let cmp_id = self.new_value_id();
                self.emit(Instruction::icmp(CompareOp::Slt, Value::Instruction(load_id), end_val, cmp_id));
                self.emit(Instruction::cond_br(Value::Instruction(cmp_id), body_bb.0, exit_bb.0));
                ...
                self.emit(Instruction::load(IRType::I64, Value::Instruction(alloca_id), load2_id));
                let one = Value::Constant(Constant::i64(1));
                let inc_id = self.new_value_id();
                self.emit(Instruction::binary(BinaryOp::Add, IRType::I64, Value::Instruction(load2_id), one, inc_id));
                self.emit(Instruction::store(Value::Instruction(inc_id), Value::Instruction(alloca_id)));
                self.emit(Instruction::br(cond_bb.0));
            ...
            Stmt::CompoundAssign { name, op, value } => {
                if let Some(&var_id) = self.variables.get(name) {
                    let load_id = self.new_value_id();
                    self.emit(Instruction::load(IRType::I64, Value::Instruction(var_id), load_id));
                    ...
                    self.emit(Instruction::binary(
                        bin_op, IRType::I64,
                        Value::Instruction(load_id), rhs, result_id
                    ));
                    
                    self.emit(Instruction::store(Value::Instruction(result_id), Value::Instruction(var_id)));
                }
            }
            ...
            Stmt::Increment { name, is_pre: _, is_increment } => {
                if let Some(&var_id) = self.variables.get(name) {
                    ...
                    let result_id = self.new_value_id();
                    self.emit(Instruction::binary(op, IRType::I64, Value::Instruction(load_id), one, result_id));
            ...
            Stmt::Switch { expr, cases, default } => {
                ...
                for (val, bb) in &case_blocks {
                    ...
                    self.emit(Instruction::icmp(CompareOp::Eq, switch_val.clone(), const_val, cmp_id));
                    
                    let next_bb = self.new_block("switch.next");
                    self.emit(Instruction::cond_br(Value::Instruction(cmp_id), bb.0, next_bb.0));
    ...
    fn lower_expr(&mut self, expr: &Expr, _expected_ty: &IRType) -> Value {
        match expr {
            ...
            Expr::BinaryOp { op, left, right } => {
                ...
                let result_id = self.new_value_id();
                self.emit(Instruction::binary(bin_op, IRType::I64, lhs, rhs, result_id));
            ...
            Expr::UnaryOp { op, expr: inner } => {
                ...
                match op {
                    UnaryOp::Neg => {
                        ...
                        let result_id = self.new_value_id();
                        self.emit(Instruction::binary(BinaryOp::Sub, IRType::I64, zero, val, result_id));
                        Value::Instruction(result_id)
                    }
                    UnaryOp::Not => {
                        let one = Value::Constant(Constant::i64(1));
                        let result_id = self.new_value_id();
                        self.emit(Instruction::binary(BinaryOp::Xor, IRType::I64, val, one, result_id));
                        Value::Instruction(result_id)
                    }
                }
            }
            ...
            Expr::Call { name, args } => {
                let arg_vals: Vec<Value> = args.iter()
                    ...
                    .collect();
                let result_id = self.new_value_id();
                self.emit(Instruction::call(IRType::I64, name, arg_vals, Some(result_id)));
                Value::Instruction(result_id)
            }
            ...
            Expr::Index { object, index } => {
                ...
                self.emit(Instruction::gep(IRType::I64, ptr, vec![idx], gep_id));
                let load_id = self.new_value_id();
                self.emit(Instruction::load(IRType::I64, Value::Instruction(gep_id), load_id));
                Value::Instruction(load_id)
            }
            ...
            Expr::Ternary { condition, then_expr, else_expr } => {
                ...
                let result_id = self.new_value_id();
                self.emit(Instruction::select(IRType::I64, cond, then_val, else_val, result_id));
                Value::Instruction(result_id)
            }
            ...
            Expr::IntCast(inner) => {
                ...
                let result_id = self.new_value_id();
                self.emit(Instruction::cast(CastOp::FPToSI, IRType::I64, val, result_id));
                Value::Instruction(result_id)
            }
            ...
            Expr::FloatCast(inner) => {
                let val = self.lower_expr(inner, &IRType::I64);
                let result_id = self.new_value_id();
                self.emit(Instruction::cast(CastOp::SIToFP, IRType::F64, val, result_id));
                Value::Instruction(result_id)
            }
