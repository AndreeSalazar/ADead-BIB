...
use super::{
    BasicBlock, Function, Instruction, Module, Type, Value, ValueId,
    BinaryOp, CompareOp, CastOp, Constant,
};
...
impl<'a> IRBuilder<'a> {
    ...
    pub fn build_select(&mut self, cond: Value, true_val: Value, false_val: Value, ty: Type) -> Value {
        let id = self.new_value_id();
