...
impl IsaCompiler {
    pub(crate) fn emit_if(&mut self, condition: &Expr, then_body: &[Stmt], else_body: Option<&[Stmt]>) {
    ...
    pub(crate) fn emit_for(&mut self, var: &str, start: &Expr, end: &Expr, body: &[Stmt]) {
        ...
        self.ir.emit(ADeadOp::Mov {
            dst: Operand::Reg(Reg::RCX), src: Operand::Reg(Reg::RAX),
        });
        self.emit_expression(end);
        self.ir.emit(ADeadOp::Mov {
            dst: Operand::Reg(Reg::R8), src: Operand::Reg(Reg::RAX),
        });
        ...
        self.ir.emit(ADeadOp::Cmp {
            left: Operand::Reg(Reg::RCX), right: Operand::Reg(Reg::R8),
        });
    ...
    pub(crate) fn emit_return(&mut self, expr: Option<&Expr>) {
        ...
        self.ir.emit(ADeadOp::Mov {
            dst: Operand::Reg(Reg::RSP), src: Operand::Reg(Reg::RBP),
        });
    ...
    pub(crate) fn emit_condition(&mut self, expr: &Expr) {
        match expr {
            Expr::Comparison { op, left, right } => {
                ...
                self.ir.emit(ADeadOp::Cmp {
                    left: Operand::Reg(Reg::RAX), right: Operand::Reg(Reg::RBX),
                });
            ...
            Expr::Bool(b) => {
                ...
                self.ir.emit(ADeadOp::Mov {
                    dst: Operand::Reg(Reg::EAX), src: Operand::Imm32(val),
                });
            }
