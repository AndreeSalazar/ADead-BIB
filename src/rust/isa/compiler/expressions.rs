...
use crate::isa::{ADeadOp, Reg, Operand, Condition};
        ...
        match expr {
            ...
            Expr::RegRead { reg_name } => {
                if let Some(reg) = Self::string_to_reg(reg_name) {
                    if reg.is_control() {
                        ...
                        self.ir.emit(ADeadOp::Mov {
                            dst: Operand::Reg(Reg::RAX), src: Operand::Reg(reg),
                        });
                    }
                }
            }
            ...
            Expr::PortIn { port } => {
                match port.as_ref() {
                    Expr::Number(p) if *p >= 0 && *p <= 255 => {
                        self.ir.emit(ADeadOp::InByte { port: Operand::Imm8(*p as i8) });
                        self.ir.emit(ADeadOp::MovZx { dst: Reg::RAX, src: Reg::AL });
                    }
                    _ => {
                        ...
                        self.ir.emit(ADeadOp::InByte { port: Operand::Reg(Reg::DX) });
                        self.ir.emit(ADeadOp::MovZx { dst: Reg::RAX, src: Reg::AL });
                    }
                }
            }
            ...
            Expr::BitwiseOp { op, left, right } => {
                ...
                if let Some(temp) = self.temp_alloc.alloc() {
                    self.ir.emit(ADeadOp::Mov {
                        dst: Operand::Reg(temp), src: Operand::Reg(Reg::RAX),
                    });
                    self.emit_expression(right);
                    self.ir.emit(ADeadOp::Mov {
                        dst: Operand::Reg(Reg::RBX), src: Operand::Reg(Reg::RAX),
                    ...
                    self.ir.emit(ADeadOp::Mov {
                        dst: Operand::Reg(Reg::RAX), src: Operand::Reg(temp),
                    });
            ...
            Expr::String(s) => {
                ...
                self.ir.emit(ADeadOp::Mov {
                    dst: Operand::Reg(Reg::RAX), src: Operand::Imm64(addr),
                });
            }
            ...
            Expr::Ternary { condition, then_expr, else_expr } => {
                ...
                self.ir.emit(ADeadOp::Cmp {
                    left: Operand::Reg(Reg::RAX), right: Operand::Imm32(0),
                });
            ...
            Expr::SizeOf(_) => {
                self.ir.emit(ADeadOp::Mov {
                    dst: Operand::Reg(Reg::RAX), src: Operand::Imm64(8),
                });
            }
