...
pub fn generate_pe_ultra_v2(
    ...
    let file_alignment = if config.minimal_alignment { 0x200 } else { 0x200 };
    let section_alignment = if config.minimal_alignment { 0x200 } else { 0x1000 };
    ...
    let aligned_code_data = ((total_code_data + file_alignment - 1) / file_alignment) * file_alignment;
    ...
    let opt_header_size = if config.strip_data_dirs { 112u16 } else { 240u16 };
    ...
    // SizeOfImage
    let size_of_image = ((0x200 + aligned_code_data + section_alignment - 1) / section_alignment) * section_alignment;
    ...
    if data_size > 0 {
        let data_rva = 0x200 + ((code_size + section_alignment - 1) / section_alignment) * section_alignment;
        ...
        let aligned_data = (((data_size + file_alignment - 1) / file_alignment) * file_alignment) as u32;
        sec_data[16..20].copy_from_slice(&aligned_data.to_le_bytes());
        sec_data[20..24].copy_from_slice(&(0x200 + aligned_code_data as u32 - data_size as u32).to_le_bytes());
...
pub fn print_size_comparison(adead_size: usize, code_size: usize) {
    ...
    println!("     ├─────────────────────────────────────────────────────┤");
    println!("     │ NASM + link      │ ~4,096 bytes│ {:.1}x más grande    │", 4096.0 / adead_size as f64);
    println!("     │ MASM + link      │ ~4,096 bytes│ {:.1}x más grande    │", 4096.0 / adead_size as f64);
    println!("     │ GCC (C)          │ ~50,000 bytes│ {:.1}x más grande   │", 50000.0 / adead_size as f64);
    println!("     │ Rust             │ ~150,000 bytes│ {:.1}x más grande │", 150000.0 / adead_size as f64);
    println!("     │ Go               │ ~2,000,000 bytes│ {:.1}x más grande│", 2000000.0 / adead_size as f64);
    println!("     └─────────────────────────────────────────────────────┘");
    println!();
...
mod tests {
    ...
    fn test_minimal_pe_size() {
        ...
        assert!(size <= 1024, "Minimal PE should be <= 1KB, got {} bytes", size);
