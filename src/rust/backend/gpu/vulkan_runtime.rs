...
impl VulkanRuntime {
    ...
    unsafe fn select_best_device(instance: &ash::Instance, devices: &[vk::PhysicalDevice]) -> vk::PhysicalDevice {
    ...
    pub unsafe fn create_buffer(&self, size: u64, usage: vk::BufferUsageFlags) -> Result<(vk::Buffer, vk::DeviceMemory), String> {
    ...
    pub unsafe fn create_buffer_with_data<T: Copy>(&self, data: &[T]) -> Result<(vk::Buffer, vk::DeviceMemory), String> {
    ...
    /// Copia buffer
    unsafe fn copy_buffer(&self, src: vk::Buffer, dst: vk::Buffer, size: u64) -> Result<(), String> {
        let alloc_info = vk::CommandBufferAllocateInfo::default()
            .command_pool(self.command_pool)
            .level(vk::CommandBufferLevel::PRIMARY)
            .command_buffer_count(1);
        
        let cmd_buffer = self.device.allocate_command_buffers(&alloc_info)
        ...
        self.device.cmd_copy_buffer(cmd_buffer, src, dst, &[copy_region]);
        ...
        self.device.queue_submit(self.queue, &submit_infos, vk::Fence::null())
    ...
    pub unsafe fn create_shader_module(&self, spirv: &[u8]) -> Result<vk::ShaderModule, String> {
        ...
        let code: Vec<u32> = spirv.chunks(4)
            .map(|chunk| {
                let mut bytes = [0u8; 4];
    ...
    pub unsafe fn create_compute_pipeline(
        ...
        let pipeline_layout = self.device.create_pipeline_layout(&layout_info, None)
    ...
    pub unsafe fn dispatch_compute(
        ...
        self.device.cmd_dispatch(cmd_buffer, workgroups.0, workgroups.1, workgroups.2);
    ...
    pub fn print_device_info(&self) {
        ...
        println!("║ Vendor ID:  0x{:04X}                                          ║", self.device_props.vendor_id);
        println!("║ Type:       {:?}                               ║", self.device_props.device_type);
        println!("║ API:        {}.{}.{}                                           ║",
        ...
        println!("╠══════════════════════════════════════════════════════════════╣");
        println!("║ Max Workgroup Size:  {:?}                     ║", self.device_props.max_compute_workgroup_size);
        println!("║ Max Invocations:     {}                                    ║", self.device_props.max_compute_workgroup_invocations);
        println!("║ Shared Memory:       {} KB                                 ║", self.device_props.max_compute_shared_memory / 1024);
        println!("╚══════════════════════════════════════════════════════════════╝");
    }
    ...
    pub fn print_metrics(&self) {
        ...
        println!("╠══════════════════════════════════════════════════════════════╣");
        println!("║ Dispatches:    {}                                            ║", self.metrics.dispatches);
        println!("║ Total time:    {:.3} ms                                      ║", self.metrics.total_time_ns as f64 / 1_000_000.0);
        println!("║ Avg time:      {:.3} µs                                      ║", avg_ns as f64 / 1_000.0);
        println!("║ Min time:      {:.3} µs                                      ║", self.metrics.min_time_ns as f64 / 1_000.0);
        println!("║ Max time:      {:.3} µs                                      ║", self.metrics.max_time_ns as f64 / 1_000.0);
        println!("╚══════════════════════════════════════════════════════════════╝");
    }
}
