...
impl BytecodeToSpirV {
    ...
    fn emit_preamble(&mut self) {
        ...
        self.emit(SpirVOp::OpTypeArray, &[array_type, self.type_float, array_size_id]);
        ...
        self.emit(SpirVOp::OpTypePointer, &[self.type_ptr_storage, 12, struct_type]);
        
        self.type_ptr_float = self.alloc_id();
        self.emit(SpirVOp::OpTypePointer, &[self.type_ptr_float, 12, self.type_float]);
        
        let ptr_func_float = self.alloc_id();
        self.emit(SpirVOp::OpTypePointer, &[ptr_func_float, 7, self.type_float]); // Function
        ...
        self.emit(SpirVOp::OpVariable, &[self.type_ptr_storage, self.var_input, 12]);
        
        self.var_output = self.alloc_id();
        self.emit(SpirVOp::OpVariable, &[self.type_ptr_storage, self.var_output, 12]);
    ...
    pub fn compile(&mut self, bytecode: &[u8]) -> Vec<u8> {
        ...
        self.emit(SpirVOp::OpEntryPoint, &[
            ...
            self.var_global_id, self.var_input, self.var_output
        ]);
        ...
        // Main function
        self.emit(SpirVOp::OpFunction, &[self.type_void, main_func, 0, self.type_func]);
    ...
    fn compile_instruction(&mut self, instr: &ADeadGpuInstr, gid: u32, const_0: u32, _acc: u32) {
        match instr.opcode {
            ...
            ADeadGpuOp::Load => {
                ...
                self.emit(SpirVOp::OpConstant, &[self.type_uint, offset, instr.operand as u32]);
                
                let idx = self.emit_result(SpirVOp::OpIAdd, self.type_uint, &[gid, offset]);
                let ptr = self.emit_result(SpirVOp::OpAccessChain, self.type_ptr_float, 
            ...
            ADeadGpuOp::LoadImm => {
                ...
                self.emit(SpirVOp::OpConstant, &[self.type_float, _val, instr.operand as u32]);
            }
