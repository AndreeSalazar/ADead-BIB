...
impl CudaCodeGen {
    ...
    pub fn add_vector_add_kernel(&mut self, n: usize) {
        let kernel = CudaKernel {
            ...
            params: vec![
                CudaParam { name: "A".to_string(), param_type: CudaType::Float, is_pointer: true },
                CudaParam { name: "B".to_string(), param_type: CudaType::Float, is_pointer: true },
                CudaParam { name: "C".to_string(), param_type: CudaType::Float, is_pointer: true },
                CudaParam { name: "n".to_string(), param_type: CudaType::Int, is_pointer: false },
            ],
            body: r#"
...
"#.to_string(),
            ...
            grid_size: (((size + block_size - 1) / block_size) as u32, 
                       ((size + block_size - 1) / block_size) as u32, 1),
        };
        self.kernels.push(kernel);
    }
    ...
    fn generate_kernel(&self, kernel: &CudaKernel) -> String {
        ...
        let params: Vec<String> = kernel.params.iter().map(|p| {
